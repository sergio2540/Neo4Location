...
public class RawData { 
	 ...
	 * a human readable time to the person, or for carefully comparing
	...
	public Map<String, String> toMap() {
		...
		map.put(Neo4LocationProperties.LATITUDE, Double.toString(mLatitude));
		map.put(Neo4LocationProperties.LONGITUDE, Double.toString(mLongitude));
		
		if (mHasAccuracy)
			map.put(Neo4LocationProperties.ACCURACY, Float.toString(mAccuracy));
		
		if (mTime == 0) {
			map.put(Neo4LocationProperties.TIMESTAMP, Float.toString(mTime));
		}
		
		if (mHasAltitude) {
			map.put(Neo4LocationProperties.ALTITUDE, Double.toString(mAltitude));
		}
		if (mHasSpeed) {
			map.put(Neo4LocationProperties.SPEED, Double.toString(mSpeed));
		}
		if (mHasBearing){
			map.put(Neo4LocationProperties.BEARING, Double.toString(mBearing));
		}
